...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Ejemplos:
  python scripts/pipeline/explore_data.py
  python scripts/pipeline/explore_data.py --csv data/processed/dataset_final.csv --output reports/eda.txt
        """.strip(),
    )
    parser.add_argument(
        "--csv",
...
def main() -> None:
    ...
    if not csv_path.exists():
        ...
        print(f"        Puedes especificar otra ruta con: python scripts/pipeline/explore_data.py --csv <ruta>", file=sys.stderr)
        sys.exit(1)
